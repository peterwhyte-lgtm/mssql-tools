...
# Server array block — e.g.  'SVR01',`n    'SVR02'
$serverArrayInner = ($serverList | ForEach-Object { "    '$_'" }) -join ",`n"

$nl = [Environment]::NewLine

# Execution mode label
$execMode = if ($Parallel) { 'Parallel (ForEach-Object -Parallel) — requires PowerShell 7+' } else { 'Sequential (foreach)' }
...
function Build-SqlScript {
    ...
    if ($SqlAuth) {
        $credBlock = $nl +
            '# SQL authentication — enter credentials when prompted' + $nl +
    ...
    # Loop body — uses single-quoted literal strings where $server must remain as-is
    $tryBody  = '        $results = Invoke-Sqlcmd ' + $invokeArgs + $nl +
                '        if ($results) {' + $nl +
                "            `$results | Format-Table -AutoSize" + $nl +
                '        } else {' + $nl +
    ...
    return ($header, $serverBlock, $sqlBlock, $credBlock, $loop, 'Write-Host "`nDone." -ForegroundColor Green') -join ($nl + $nl)
}
...
function Build-PsScript {
    ...
    $prereqNote   = if ($Ssh) { 'SSH key-based auth must be configured on target servers.' } else { "PowerShell remoting must be enabled on each target server.$nl" +
                                "                    Enable with (run as admin on the target): Enable-PSRemoting -Force" }
    $computerFlag = if ($Ssh) { '-HostName $server' } else { '-ComputerName $server' }
    ...
    # Embed the script content inside a scriptblock
    $scriptBlock = '$scriptBlock = {' + $nl + $scriptContent.TrimEnd() + $nl + '}'

    $tryBody = "        Invoke-Command $computerFlag -ScriptBlock `$scriptBlock"
    ...
    return ($header, $serverBlock, $scriptBlock, $loop, 'Write-Host "`nDone." -ForegroundColor Green') -join ($nl + $nl)
}
