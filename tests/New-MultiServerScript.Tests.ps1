...
<#
Pester tests for helpers/multi-server-query/New-MultiServerScript.ps1
...
Run from repo root:
    Invoke-Pester tests/New-MultiServerScript.Tests.ps1 -Output Detailed
#>

BeforeAll {
    $generator  = (Resolve-Path "$PSScriptRoot\..\helpers\multi-server-query\New-MultiServerScript.ps1").Path
    $hasSqlMod  = $null -ne (Get-Module -Name SqlServer -ListAvailable -ErrorAction SilentlyContinue)

    # Minimal PS script used as source content in most tests
    $psSource   = Join-Path $TestDrive 'sample.ps1'
    Set-Content -Path $psSource -Value 'Write-Host "hello from $env:COMPUTERNAME"' -Encoding UTF8

    # Helper: run generator and return output file content
    function Invoke-Generator {
        param([hashtable]$Params)
        $out = Join-Path $TestDrive ([System.IO.Path]::GetRandomFileName() + '.ps1')
...
Describe 'Input validation' {
    ...
    It 'throws on a missing ScriptPath' {
        { & $generator -ScriptPath 'C:\does\not\exist.ps1' -Servers 'SVR01' 2>&1 } | Should -Throw
    }

    It 'throws on an unsupported file extension' {
        $f = New-Item (Join-Path $TestDrive 'test.txt') -ItemType File -Value 'hello'
        { & $generator -ScriptPath $f.FullName -Servers 'SVR01' 2>&1 } | Should -Throw
    }

    It 'throws on an empty Servers string' {
        { & $generator -ScriptPath $psSource -Servers '  ,  ,  ' 2>&1 } | Should -Throw
    }
...
Describe 'SQL here-string guard' {
    ...
    It "throws when SQL contains a line starting with '@ (would terminate the here-string)" {
        ...
        { & $generator -ScriptPath $bad -Servers 'SVR01' 2>&1 } | Should -Throw -ExpectedMessage "*here-string*"
    }
...
Describe 'Cross-validation flag warnings' {
    ...
    It 'emits a warning but does not throw when -SqlAuth is used with a .ps1 file' {
        $w = & $generator -ScriptPath $psSource -Servers 'SVR01' -SqlAuth -OutputFile (Join-Path $TestDrive 'warn.ps1') 2>&1 |
    ...
    It 'warns when the source PS script references $PSScriptRoot' {
        ...
        Set-Content -Path $repoDependent -Value 'Import-Module $PSScriptRoot\..\..\helpers\helper.psm1'
        $w = & $generator -ScriptPath $repoDependent -Servers 'SVR01' -OutputFile (Join-Path $TestDrive 'repo-warn.ps1') 2>&1 |
            Where-Object { $_ -is [System.Management.Automation.WarningRecord] }
        $w | Should -Not -BeNullOrEmpty
    }
}
