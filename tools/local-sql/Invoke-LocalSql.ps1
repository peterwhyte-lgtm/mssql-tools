...
.EXAMPLE
.\tools\local-sql\Invoke-LocalSql.ps1 -Query "SELECT name FROM sys.databases ORDER BY name"
.\tools\local-sql\Invoke-LocalSql.ps1 -Query "SELECT @@VERSION" -ServerInstance PROD01
.\tools\local-sql\Invoke-LocalSql.ps1 -Query "SELECT * FROM sys.dm_exec_requests" -OutputFormat Csv
#>
param(
    [Parameter(Mandatory)]
    [string]$Query,
