<#
.SYNOPSIS
Scheduled SQL Server performance counter collector.

.DESCRIPTION
Captures a curated set of sys.dm_os_performance_counters values (buffer pool,
memory, throughput, locks, connections) and appends to a rolling daily CSV.
Rate-based counters are cumulative — diff adjacent snapshots for interval rates.

Output: output-files\collectors\perfmon\<server>-<YYYYMMDD>.csv
...
$sqlScript = Join-Path $PSScriptRoot 'perfmon.sql'

if (-not $OutputRoot) { $OutputRoot = Join-Path $repoRoot 'output-files\collectors\perfmon' }
...
Write-Log "Starting perfmon collection — $ServerInstance"
...
if ($invokeSqlcmd) {
    ...
    $tmp = [IO.Path]::Combine([IO.Path]::GetTempPath(), "pfm-$(Get-Date -Format 'yyyyMMddHHmmss').tmp.csv")
...
if (-not $rows -or $rows.Count -eq 0) {
    Write-Log 'No rows returned.' 'WARN'; exit 0
}
...
Write-Log "OK — $($rows.Count) counter(s) $verb to $([IO.Path]::GetFileName($csvPath))"
