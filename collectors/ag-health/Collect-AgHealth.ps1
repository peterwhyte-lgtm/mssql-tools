...
Write-Log "Starting AG health collection — $ServerInstance"
...
if ($agRows.Count -eq 0) {
    Write-Log 'No AG configured — skipping write.'
    exit 0
}
...
Write-Log "OK — $($agRows.Count) row(s) $verb to $([IO.Path]::GetFileName($csvPath))"
