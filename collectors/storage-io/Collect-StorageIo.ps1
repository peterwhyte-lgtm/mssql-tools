...
.DESCRIPTION
Captures sys.dm_io_virtual_file_stats per database file. Counters are cumulative
since SQL Server start — diff adjacent snapshots for interval read/write rates
and latency. Includes sqlserver_start_time for restart detection.

Output: output-files\collectors\storage-io\<server>-<YYYYMMDD>.csv
...
$sqlScript = Join-Path $PSScriptRoot 'storage-io.sql'

if (-not $OutputRoot) { $OutputRoot = Join-Path $repoRoot 'output-files\collectors\storage-io' }
...
Write-Log "Starting storage-io collection — $ServerInstance"
...
if ($invokeSqlcmd) {
    ...
    $tmp = [IO.Path]::Combine([IO.Path]::GetTempPath(), "io-$(Get-Date -Format 'yyyyMMddHHmmss').tmp.csv")
...
Write-Log "OK — $($rows.Count) file(s) $verb to $([IO.Path]::GetFileName($csvPath))"
