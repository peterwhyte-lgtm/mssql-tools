...
.DESCRIPTION
Executes database-admin\sql-scripts\backups\Generate-DiffBackupScript.sql and writes the result to
...
.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\backup-automation\Generate-DiffBackupScript.ps1

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\backup-automation\Generate-DiffBackupScript.ps1 -ServerInstance PROD01\SQL2019
#>
param(
...
$sqlScript = Join-Path $repoRoot 'database-admin\sql-scripts\backups\Generate-DiffBackupScript.sql'
