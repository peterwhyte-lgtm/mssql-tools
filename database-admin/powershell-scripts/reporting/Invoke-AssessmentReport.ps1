...
.EXAMPLE
.\database-admin\powershell-scripts\reporting\Invoke-AssessmentReport.ps1 -ServerInstance PROD01\SQL2019

.EXAMPLE
# Reuse an existing collection folder
.\database-admin\powershell-scripts\reporting\Invoke-AssessmentReport.ps1 -FolderPath ".\output-files\healthcheck\PROD01-20260531-140000" -AssessedBy "Peter Whyte"
#>
...
if ($FolderPath) {
    ...
    $hcScript = Join-Path $repoRoot 'database-admin\powershell-scripts\reporting\Invoke-HealthCheckCollection.ps1'
...
$scoreSql = Join-Path $repoRoot 'database-admin\sql-scripts\monitoring\Get-InstanceConfigurationScore.sql'
