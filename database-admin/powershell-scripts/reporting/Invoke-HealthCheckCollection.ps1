...
.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Invoke-HealthCheckCollection.ps1

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Invoke-HealthCheckCollection.ps1 -ServerInstance MYSERVER\INST01

.EXAMPLE
# Collect then immediately review
$folder = pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Invoke-HealthCheckCollection.ps1 -ServerInstance . 6>&1 | Select-String 'Output folder' | ForEach-Object { $_.Line -replace '.*: ' }
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Review-HealthCheckOutput.ps1 -FolderPath $folder
#>
...
$scripts = @(
    [PSCustomObject]@{
        Label = 'server-info'
        Paths = @('database-admin\sql-scripts\monitoring\Get-VersionAndEdition.sql')
    }
    [PSCustomObject]@{
        Label = 'os-hardware'
        Paths = @('database-admin\sql-scripts\monitoring\Get-OsAndHardwareInfo.sql')
    }
    [PSCustomObject]@{
        Label = 'database-health'
        Paths = @('database-admin\sql-scripts\monitoring\Get-DatabaseHealth.sql')
    }
    [PSCustomObject]@{
        Label = 'database-sizes'
        Paths = @('database-admin\sql-scripts\monitoring\Get-DatabaseSizesAndFreeSpace.sql')
    }
    [PSCustomObject]@{
        Label = 'database-files'
        Paths = @('database-admin\sql-scripts\monitoring\Get-DatabaseFilesDetail.sql')
    }
    [PSCustomObject]@{
        Label = 'backup-times'
        Paths = @('database-admin\sql-scripts\backups\Get-LastDatabaseBackupTimes.sql')
    }
    [PSCustomObject]@{
        Label = 'backup-coverage'
        Paths = @('database-admin\sql-scripts\backups\Get-BackupCoverage.sql')
    }
    [PSCustomObject]@{
        Label = 'tlog-usage'
        Paths = @('database-admin\sql-scripts\monitoring\Get-TransactionLogSizeAndUsage.sql')
    }
    [PSCustomObject]@{
        Label = 'memory-config'
        Paths = @('database-admin\sql-scripts\monitoring\Get-MemoryConfigurationAndUsage.sql')
    }
    [PSCustomObject]@{
        Label = 'wait-stats'
        Paths = @('database-admin\sql-scripts\performance\Get-WaitStatistics.sql')
    }
    [PSCustomObject]@{
        Label = 'active-sessions'
        Paths = @('database-admin\sql-scripts\performance\Get-ActiveSessions.sql')
    }
    [PSCustomObject]@{
        Label = 'tempdb-usage'
        Paths = @('database-admin\sql-scripts\monitoring\Get-TempdbUsage.sql')
    }
    [PSCustomObject]@{
        Label = 'job-failures'
        Paths = @('database-admin\sql-scripts\monitoring\Get-SqlAgentJobFailureSummary.sql')
    }
    [PSCustomObject]@{
        Label = 'recent-errors'
        Paths = @('database-admin\sql-scripts\monitoring\Get-RecentErrorLogEntries.sql')
    }
    [PSCustomObject]@{
        Label = 'dbcc-checkdb'
        Paths = @('database-admin\sql-scripts\monitoring\Get-LastDbccCheckdb.sql')
    }
    [PSCustomObject]@{
        Label = 'suspect-pages'
        Paths = @('database-admin\sql-scripts\monitoring\Get-SuspectPages.sql')
    }
    [PSCustomObject]@{
        Label = 'io-usage'
        Paths = @('database-admin\sql-scripts\performance\Get-DatabaseIoUsage.sql')
    }
    [PSCustomObject]@{
        Label = 'disk-space'
        Paths = @('database-admin\sql-scripts\monitoring\Get-DiskSpace.sql')
    }
    [PSCustomObject]@{
        Label = 'growth-risk'
        Paths = @('database-admin\sql-scripts\monitoring\Get-DatabaseGrowthRisk.sql')
    }
    [PSCustomObject]@{
        Label = 'security-surface-area'
        Paths = @('database-admin\sql-scripts\security\Get-DatabaseMailAndXpCmdShell.sql')
    }
    [PSCustomObject]@{
        Label = 'weak-logins'
        Paths = @('database-admin\sql-scripts\security\Get-WeakLoginSettings.sql')
    }
    [PSCustomObject]@{
        Label = 'missing-indexes'
        Paths = @('database-admin\sql-scripts\performance\Get-MissingIndexes.sql')
    }
    [PSCustomObject]@{
        Label = 'tempdb-config'
        Paths = @('database-admin\sql-scripts\monitoring\Get-TempDbConfiguration.sql')
    }
    [PSCustomObject]@{
        Label = 'plan-cache'
        Paths = @('database-admin\sql-scripts\performance\Get-PlanCacheHealth.sql')
    }
    [PSCustomObject]@{
        Label = 'linked-server-security'
        Paths = @('database-admin\sql-scripts\security\Get-LinkedServerSecurity.sql')
    }
    [PSCustomObject]@{
        Label = 'vlf-count'
        Paths = @('database-admin\sql-scripts\monitoring\Get-VlfCount.sql')
    }
    [PSCustomObject]@{
        Label    = 'maintenance-jobs'
        Paths    = @('database-admin\sql-scripts\maintenance\Get-MaintenanceJobStatus.sql')
        Database = 'msdb'
    }
)
...
Write-Host ''
Write-Host "  CLI review: .\database-admin\powershell-scripts\reporting\Review-HealthCheckOutput.ps1" -ForegroundColor DarkGray
Write-Host ('─' * 64) -ForegroundColor DarkCyan
Write-Host ''
