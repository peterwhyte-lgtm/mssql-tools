...
.DESCRIPTION
Wrapper around database-admin\sql-scripts\performance\Get-ActiveRequests.sql. When -IncludePlan is set, runs
database-admin\sql-scripts\performance\Get-ActiveRequestsWithPlan.sql and writes each session's query_plan
...
.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-ActiveRequests.ps1

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-ActiveRequests.ps1 -ServerInstance PROD01\SQL2019 -IncludePlan

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-ActiveRequests.ps1 -ServerInstance . -Append -OutputPath .\output-files\diagnostics\active-requests\rolling.csv
#>
...
$sqlScript = Join-Path $repoRoot "database-admin\sql-scripts\performance\$sqlName"
