...
.DESCRIPTION
Wrapper around database-admin\sql-scripts\performance\Get-BlockingChains.sql. When -IncludePlan is set,
runs database-admin\sql-scripts\performance\Get-BlockingChainsWithPlan.sql and writes each session's
...
.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-BlockingChains.ps1

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-BlockingChains.ps1 -ServerInstance PROD01\SQL2019 -IncludePlan

.EXAMPLE
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\database-admin\powershell-scripts\reporting\Get-BlockingChains.ps1 -ServerInstance . -Append -OutputPath .\output-files\diagnostics\blocking-chains\rolling.csv
#>
