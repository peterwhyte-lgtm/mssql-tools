...
.\database-admin\powershell-scripts\maintenance\Invoke-MaintenanceDeployment.ps1 `
...
if ($Mode -in 'All', 'Backup') {
    $sql = Get-Content (Join-Path $repoRoot 'database-admin\sql-scripts\maintenance\Generate-BackupJobs.sql') -Raw
...
if ($Mode -in 'All', 'Index') {
    $sql = Get-Content (Join-Path $repoRoot 'database-admin\sql-scripts\maintenance\Generate-IndexMaintenanceJobs.sql') -Raw
    $sql = Set-SqlDeclare $sql 'JobOwner'              $JobOwner
...
if ($Mode -in 'All', 'Maintenance') {
    $sql = Get-Content (Join-Path $repoRoot 'database-admin\sql-scripts\maintenance\Generate-MaintenanceJobs.sql') -Raw
    $sql = Set-SqlDeclare $sql 'JobOwner' $JobOwner
...
Write-Host ''
Write-Host "  Verify with: .\database-admin\powershell-scripts\multi-server\MultiServer-GetMaintenanceJobStatus.ps1 -Servers `"$Servers`"" -ForegroundColor DarkGray
Write-Host ''
