...
.DESCRIPTION
Captures active blocking chains from sys.dm_exec_requests. On quiet servers this
produces no output. On busy servers each run appends one batch of rows to the daily
CSV. Designed for SQL Agent / Task Scheduler silent execution.

Output: output-files\collectors\blocking\<server>-<YYYYMMDD>.csv
...
$sqlScript = Join-Path $PSScriptRoot 'blocking.sql'

if (-not $OutputRoot) { $OutputRoot = Join-Path $repoRoot 'output-files\collectors\blocking' }
